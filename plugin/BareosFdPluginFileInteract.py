...
class BareosFdPluginFileInteract  (BareosFdPluginBaseclass):
    ...
    def handle_backup_file(self,context, savepkt):
        ...
        if ( savepkt.type == bFileType['FT_REG'] ):
            ...
            #JobMessage(context, bJobMessageType['M_ABORT'], "Option Plugin file interact on" + savepkt.fname + "\n");
            JobMessage(context, bJobMessageType['M_WARNING'], "Option Plugin file interact on" + savepkt.fname + "\n");
            # add your stuff here

        #return bRCs['bRC_OK'];
        return bRCs['bRC_Stop'];
        #return bRCs['bRC_Term'];
